...
def qtrain(train_loader, criterion, optimizer, scheduler, epoch,model, args, layers, rew_layers, eps):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
...
def quantized_retrain(criterion, optimizer, scheduler):
    ...
    if args.fwd_bits < 32:
        new_model = quant.duplicate_model_with_quant(model, bits=args.fwd_bits, overflow_rate=args.overflow_rate,
                                                 counter=args.n_sample, type=args.quant_method)
        model.load_state_dict(new_model.state_dict())
    
    #retrain
    for epoch in range(1, args.epochs + 1):
        idx_loss_dict = qtrain(train_loader, criterion, optimizer, scheduler, epoch,model, args,layers=None, rew_layers=None, eps=None)
        t_loss, prec1 = test(model, criterion, test_loader)

        if prec1 > max(best_prec1):
            print("\n>_ Got better accuracy, saving model with accuracy {:.3f}% now...\n".format(prec1))
            torch.save(model.state_dict(), "./model_retrained/quantized_cifar100_{}{}_retrained_acc_{:.3f}_{}_{}.pt".format(
                args.arch, args.depth, prec1, args.config_file, args.sparsity_type))
            print("\n>_ Deleting previous model file with accuracy {:.3f}% now...\n".format(max(best_prec1)))
            if len(best_prec1) > 1:
                os.remove("./model_retrained/quantized_cifar100_{}{}_retrained_acc_{:.3f}_{}_{}.pt".format(
                    args.arch, args.depth, max(best_prec1), args.config_file, args.sparsity_type))
